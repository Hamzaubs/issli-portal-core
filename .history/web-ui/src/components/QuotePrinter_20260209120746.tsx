...
export const QuotePrinter = ({ items, clientName, onClose, onConfirm }: QuoteProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-slate-200 rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col animate-in fade-in zoom-in-95">
        ...
        <div className="p-4 bg-white border-b border-slate-300 flex justify-between items-center rounded-t-xl shrink-0">
           ...
           <div className="flex gap-3">
             <button onClick={onClose} className="px-4 py-2 text-slate-500 font-bold hover:bg-slate-100 rounded-lg">Fermer</button>
             {onConfirm && (
           ...
           <div ref={componentRef} className="bg-white w-[210mm] min-h-[297mm] p-[15mm] shadow-xl text-slate-900 relative">
                      ...
                      {items.map((item, i) => (
                          <tr key={i}>
                              <td className="py-4 font-bold text-slate-700">
                                  {item.product.name}
                                  <div className="text-[10px] text-slate-400 font-mono">{item.product.internalSku}</div>
                              </td>
              ...
              <div className="flex justify-end">
                  <div className="w-1/2">
                      ...
                      <p className="text-[10px] text-slate-400 text-right mt-2 italic">
                          * Ce devis n'est pas une facture. Les prix sont valables dans la limite des stocks disponibles.
                      </p>
                  </div>
              </div>
           </div>
