...
import { RefreshCw, ArrowUpRight, ArrowDownLeft, RotateCcw, User, FileText, Banknote, CreditCard, Truck, Printer } from 'lucide-react';
...
interface Movement {
    ...
    // We add these for reconstruction of receipt
    unitPrice?: number; 
}
...
export const MovementHistory = () => {
    ...
    const handlePrint = (m: Movement) => {
        if (m.type === 'QUOTE') {
            ...
            const quoteItem = {
                product: { name: m.productName, internalSku: m.productSku, measureUnit: m.measureUnit },
                qty: m.quantity,
                unitPrice: m.amount / m.quantity // Calculate unit price derived from total
            };
            setPrintQuoteData({
                items: [quoteItem],
    ...
    return (
        <div className="h-full flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in">
                         ...
                         movements.map(m => (
                            <tr key={m.id} className="hover:bg-slate-50/80 transition-colors group">
                                ...
                                <td className="p-4 text-right flex justify-end gap-2">
                                    {/* ✅ PRINT BUTTON */}
                                    <button onClick={() => handlePrint(m)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Imprimer Ticket/Devis">
                                        <Printer size={16}/>
                                    </button>

                                    {/* VOID BUTTON */}
                                    {m.type !== 'QUOTE' && !m.productName.includes('ANNULATION') && (
                                        <button onClick={() => handleVoid(m)} className="opacity-0 group-hover:opacity-100 p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Annuler">
                                            <RotateCcw size={16}/>
                                        </button>
                                    )}
                                </td>
            ...
            {printQuoteData && (
                <QuotePrinter 
                    items={printQuoteData.items} 
                    clientName={printQuoteData.clientName} 
                    onClose={() => setPrintQuoteData(null)}
                />
            )}
        </div>
    );
