...
export const LegalDashboard = () => {
  ...
  // ✅ Removed 'CLIENTS_PAGE' from viewMode since it has its own Route now
  ...
  useEffect(() => { if (viewMode === 'DASHBOARD' && activeTab === 'invoices') fetchData(); }, [viewMode, pagination.page, debouncedSearch, activeTab]);
  ...
  const formatMAD = (n: any) => new Intl.NumberFormat('fr-MA', { style: 'currency', currency: 'MAD' }).format(Number(n));

  // --- 5. Conditional Views ---
  if (viewMode === 'ANALYTICS') return <LegalAnalytics onBack={() => setViewMode('DASHBOARD')} />;

  // ❌ Removed ClientsDebtPage check because it is now accessed via Sidebar Route
    ...
    <div className="p-8 max-w-[1600px] mx-auto min-h-screen bg-slate-50/50 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex gap-3 relative z-10">
          ...
          {/* ❌ REMOVED "Clients & Dettes" BUTTON (Now in Sidebar) */}
      ...
      {activeTab === 'invoices' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          {/* ✅ UPDATED: Clicking this card now navigates to the Clients Route */}
