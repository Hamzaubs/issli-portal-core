                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {loading ? (
                            ...
                            <tr><td colSpan={8} className="p-8 text-center text-slate-400">Aucun historique récent.</td></tr>
                        ) : (
                            movements.map(m => (
                                <tr key={m.id} className="hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td className="p-4 text-slate-600 font-bold">
                                        {m.clientName !== '-' ? (
                                            <span className="flex items-center gap-1.5 text-blue-900 bg-blue-50 px-2 py-1 rounded w-fit text-xs">
                                    ...
                                    <td className={`p-4 text-right font-bold ${m.type === 'RETURN' || m.amount < 0 ? 'text-red-500' : 'text-slate-700'}`}>
                                        {m.amount !== 0 ? formatMAD(m.amount) : '-'}
                                    </td>
                                    <td className="p-4 text-right">
                                        {/* Hide VOID button for quotes or already cancelled items */}
                                        {m.type !== 'QUOTE' && !m.productName.includes('ANNULATION') && (
                                            <button 
                                                onClick={() => handleVoid(m)}
                                                className="opacity-0 group-hover:opacity-100 p-1.5 bg-red-50 text-red-600 rounded hover:bg-red-100 transition-all text-xs font-bold flex items-center gap-1 ml-auto"
                                                title="Annuler ce mouvement (Contre-passation)"
                                            >
