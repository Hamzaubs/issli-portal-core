...
// ✅ Strict Interface Matching Backend
interface ProductB {
  id: string; 
  name: string; 
  internalSku: string; 
  ...
  sellingPrice: number; 
  quantity: number;
  measureUnit: 'UNIT' | 'METER' | 'KG' | 'LITER' | 'U' | 'M' | 'KG' | 'L'; 
...
export const StockDashboard = ({ user }: { user: any }) => {
  ...
  // ✅ LOAD FROM SECURE INTERNAL ENDPOINT
  useEffect(() => {
    client.get('/internal/products') // Updated to hit InternalController
  ...
  const handleTransaction = async () => {
    ...
    // 1. Client Requirement for Credit
    ...
    // 3. Pre-Flight Safety Check (Frontend)
    ...
    try {
      ...
      await client.post('/internal/transactions', { 
          ...
          userId: user.id, 
          quantity: Math.floor(Number(transactionQty)), // Ensure Integer
          type,
  ...
  const handlePrintQuote = async () => {
      try {
          await Promise.all(quoteCart.map(item => 
              client.post('/internal/transactions', { 
      ...
      } catch (err) { console.error("⚠️ Erreur sauvegarde devis", err); }
  ...
  const activeColor = quoteMode ? 'amber' : returnMode ? 'red' : inventoryMode ? 'purple' : 'emerald';
  ...
  return (
    <div className="h-screen flex bg-slate-100 overflow-hidden font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col border-r border-slate-200 bg-[#F8FAFC]">
        <div className="h-20 px-6 bg-white border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
           ...
           <div className="flex items-center gap-3">
               ...
               {/* NEW PRODUCT BUTTON */}
               {user.role === 'SUPER_ADMIN' && !historyMode && !inventoryMode && (
                   <button onClick={() => { setEditingProduct(null); setShowFormModal(true); }} className="p-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-500 transition-colors shadow-lg shadow-emerald-200"><Plus size={20} /></button>
               )}

               {user.role === 'SUPER_ADMIN' ? (
                   <Link to="/" className="p-3 bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-colors shadow-lg" title="Menu Principal">
                       <Grid size={20} />
                   </Link>
               ) : (
                   <button onClick={() => { localStorage.clear(); window.location.href='/'; }} className="p-3 bg-slate-200 text-slate-600 rounded-xl hover:bg-red-100 hover:text-red-600" title="Quitter">
                       <LogOut size={20} />
                   </button>
               )}
           </div>
        ...
        <div className="flex-1 overflow-auto p-6 relative">
            {historyMode ? <MovementHistory /> : inventoryMode ? (
                <div className="h-full flex flex-col bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden animate-in fade-in">
                    ...
                    <div className="p-12 text-center text-slate-400 italic">Tableau d'inventaire prêt (Mode sécurisé)</div>
                </div>
            ) : (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {filteredProducts.map(p => (
                        <div key={p.id} onClick={() => { setSelectedProduct(p); setTransactionQty(1); }}
                            ...
                            {user.role === 'SUPER_ADMIN' && (
                                <button onClick={(e) => { e.stopPropagation(); setEditingProduct(p); setShowFormModal(true); }} 
                                    className="absolute top-2 right-2 p-1.5 bg-slate-100 rounded-lg text-slate-400 hover:text-emerald-600 opacity-0 group-hover:opacity-100 transition-opacity"><Pencil size={14} /></button>
                            )}
                            <div>
                ...
                {selectedProduct ? (
                   <div className="flex-1 flex flex-col animate-in slide-in-from-right-4 duration-300">
                        ...
                        {!quoteMode && (
                            <div className="bg-white p-3 rounded-xl border border-slate-200 mb-4 shadow-sm">
                                ...
                                {paymentMethod === 'CREDIT' && (
                                    <div className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1 bg-red-50 p-2 rounded">
                                        <AlertCircle size={12}/> {returnMode ? "Ce retour réduira la dette du client." : "Cette vente sera ajoutée à la dette du client."}
                                    </div>
                                )}
                            </div>
                        )}
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                          <div className="text-sm font-bold bg-amber-50 p-4 rounded-xl text-amber-600 border border-amber-200 w-full mb-4">Mode Devis Actif</div>
                          <button onClick={handlePrintQuote} className="w-full py-4 bg-amber-500 hover:bg-amber-600 text-white font-black rounded-xl shadow-lg flex items-center justify-center gap-2"><Printer size={20} /> IMPRIMER DEVIS</button>
                      </div>
                ) : (
      ...
      {showClientSelector && (
          <ClientSelector mode="INTERNAL" onSelect={(c: any) => { setActiveClient(c); setShowClientSelector(false); }} onClose={() => setShowClientSelector(false)} />
      )}

      {/* ✅ SEPARATE PRODUCT FORM (STOCK B) */}
      {showFormModal && (
         <ProductForm 
            initialData={editingProduct}
            onCancel={() => { setShowFormModal(false); setEditingProduct(null); }} 
            onSuccess={() => { setShowFormModal(false); setEditingProduct(null); setRefresh(p => p+1); }} 
         />
      )}
