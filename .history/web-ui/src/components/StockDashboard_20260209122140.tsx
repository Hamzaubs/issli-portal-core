...
export const StockDashboard = ({ user }: { user: any }) => {
  ...
  // ✅ HELPER: Colors for main UI elements (Explicit strings for Tailwind JIT)
  const activeColorClass = quoteMode ? 'bg-amber-50 text-amber-600' 
    : returnMode ? 'bg-red-50 text-red-600' 
    : inventoryMode ? 'bg-purple-50 text-purple-600' 
    : 'bg-emerald-50 text-emerald-600';

  const activeBorderClass = quoteMode ? 'border-amber-500' 
    : returnMode ? 'border-red-600' 
    : inventoryMode ? 'border-purple-600' 
    : 'border-emerald-600';
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {filteredProducts.map(p => (
                        <div key={p.id} onClick={() => { setSelectedProduct(p); setTransactionQty(1); }}
                            ...
                            ${selectedProduct?.id === p.id ? `${activeBorderClass} ring-4 ${quoteMode ? 'ring-amber-500/20' : returnMode ? 'ring-red-500/20' : 'ring-emerald-500/20'}` : 'border-transparent hover:border-slate-300'}`}>
      ...
      {!inventoryMode && !historyMode && (
          <div className="w-[400px] xl:w-[450px] bg-white border-l border-slate-200 shadow-2xl flex flex-col z-20">
              ...
              <div className="flex-1 p-6 flex flex-col">
                <div className="mb-4">
                  <button onClick={() => setShowClientSelector(true)} className={`w-full p-4 rounded-xl border-2 border-dashed flex items-center justify-between transition-all ${activeClient ? `${activeBorderClass} ${activeColorClass}` : 'border-slate-200 text-slate-400 hover:border-slate-300'}`}>
                ...
                {selectedProduct ? (
                   <div className="flex-1 flex flex-col animate-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="grid grid-cols-2 gap-4 h-20">
                            ...
                            {/* ✅ FIXED: HARDCODED CLASSES TO ENSURE VISIBILITY IN DEVIS MODE */}
                            <button 
                                onClick={handleTransaction} 
                                disabled={submitting} 
                                className={`h-full rounded-2xl font-black text-xl shadow-xl flex items-center justify-center gap-3 transition-colors text-white ${
                                    quoteMode ? 'bg-amber-500 hover:bg-amber-600' : 
                                    returnMode ? 'bg-red-600 hover:bg-red-700' : 
