...
const InternalManager: React.FC = () => {
    ...
    const debouncedFetch = useCallback(debounce((q: string, p: number) => fetchClients(q, p), 300), []);

    useEffect(() => { debouncedFetch(search, page); }, [search, page]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowForm(false); setEditingId(null); setFormData({ name: '', ice: '', address: '', phone: '' });
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await client.delete(`/internal/clients/${id}`); fetchClients(search, page); }
        catch (error: any) { alert("Erreur: " + error.response?.data?.error); }
    };
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto min-h-screen bg-slate-50">
            {/* ... (Header and Search bars remain same as before) ... */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div><h1 className="text-3xl font-black text-slate-900 flex items-center gap-3 tracking-tight"><Users className="text-emerald-600" size={32}/> CLIENTS <span className="text-slate-300 font-light">|</span> <span className="text-emerald-600">INTERNE</span></h1></div>
                <button onClick={() => { setEditingId(null); setFormData({ name: '', ice: '', address: '', phone: '' }); setShowForm(true); }} className="px-6 py-3 rounded-xl text-white font-bold shadow-lg bg-emerald-600 hover:bg-emerald-700 flex items-center gap-2"><Plus size={20}/> Nouveau Client</button>
            </div>

            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex gap-4 items-center">
                <div className="flex-1 relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20}/><input type="text" placeholder="Rechercher..." className="w-full pl-12 pr-4 py-3 bg-slate-50 border-none rounded-lg outline-none font-medium" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} /></div>
                 ...
                 clients.map(client => (
                    <div key={client.id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 hover:shadow-md transition-all group relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4 mt-4">
                            <div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 font-black text-xl">{client.name.charAt(0).toUpperCase()}</div>
                            <div className="flex gap-2">
                                {/* ✅ VIEW BUTTON HOOKED UP */}
            ...
            {/* ✅ PROFILE MODAL */}
            {viewingClientId && <ClientProfile clientId={viewingClientId} onClose={() => setViewingClientId(null)} />}
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div><label className="text-xs font-bold text-slate-500 uppercase">Nom *</label><input required className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold outline-none" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} /></div>
