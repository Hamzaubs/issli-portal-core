...
export const TransactionForm: React.FC<Props> = ({ onClose, onSuccess }) => {
    ...
    useEffect(() => {
        // Load Data - Using the new SECURE endpoints
        const load = async () => {
            try {
                // ✅ Fetching from safe internal endpoints
                const [p, c] = await Promise.all([
                    client.get('/internal/products'),
                    client.get('/internal/clients') // Assuming this exists or falls back
                ]);
                setProducts(p.data);
                setClients(c.data || []); 
            } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await client.post('/internal/transactions', {
                type,
                productId,
                quantity: Math.floor(Number(quantity)), // Force Integer
                clientId: (type === 'SALE_CASH' || type === 'RETURN') ? clientId : undefined,
                paymentMethod: type === 'SALE_CASH' ? paymentMethod : undefined,
                // We send userId typically via session/token or a context provider, assuming handled by API middleware or user context if available here
            });
