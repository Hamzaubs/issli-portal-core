...
import { 
    ...
    FileText, ArrowRight, ChevronRight, Calculator, UserPlus
} from 'lucide-react';
...
// Helper: Debounce (No external lib required)
...
export const InternalQuoteWizard: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    // 1️⃣ CLIENT SEARCH ENGINE (Big Data Optimized)
    ...
    // 2️⃣ PRODUCT SEARCH (Load on Mount for now, or optimize later)
    useEffect(() => {
    ...
    // Local filter for products (since they are usually < 2000 items)
                        ...
                        {!showClientForm ? (
                            <div className="max-w-2xl mx-auto w-full flex flex-col h-full">
                                ...
                                <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                    {loading ? (
                                        <div className="text-center py-10 text-slate-400">Recherche en cours...</div>
                                    ) : clients.length > 0 ? (
                                        clients.map(c => (
                                            <button 
                                                key={c.id} 
                                        ...
                                        <div className="text-center py-10">
                                            ...
                                            <button onClick={() => setShowClientForm(true)} className="px-6 py-2 bg-slate-800 text-white rounded-lg font-bold hover:bg-slate-700 shadow-lg flex items-center gap-2 mx-auto">
                                                <UserPlus size={18}/> Créer Nouveau Client
                                            </button>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="max-w-md mx-auto w-full bg-white p-8 rounded-2xl shadow-xl border border-slate-200">
                                ...
                                <form onSubmit={handleCreateClient} className="space-y-4">
                                    <div>
                                        ...
                                        <input required autoFocus type="text" className="w-full p-3 border-2 border-slate-200 rounded-xl outline-none focus:border-amber-500 font-bold" 
                                    ...
                                    <div>
                                        ...
                                        <input type="text" className="w-full p-3 border-2 border-slate-200 rounded-xl outline-none focus:border-amber-500 font-medium" 
                                    ...
                                    <div className="flex gap-3 pt-4">
                                        ...
                                        <button disabled={loading} className="flex-1 py-3 bg-amber-500 text-white font-bold rounded-xl hover:bg-amber-600 shadow-lg">
                                            {loading ? '...' : 'Créer & Continuer'}
                                        </button>
                                    </div>
                                </form>
                            </div>
