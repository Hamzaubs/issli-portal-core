...
interface InvoiceItem {
  ...
  vatRateSnapshot?: number; // ✅ Vital for Historical Accuracy
...
export const InvoicePrint = ({ invoice, onClose }: { invoice: Invoice; onClose: () => void }) => {
  ...
  // ✅ ROBUST VAT LOGIC (Uses Snapshot first, falls back to Product)
  const vatBreakdown = useMemo(() => {
    ...
    (invoice.items || []).forEach(item => {
        ...
        const price = Number(item.unitPrice || item.unitPriceHT || 0);
        const qty = Number(item.quantity);
        
        const lineBase = Math.abs(price * qty); 
        const lineVat = lineBase * rate;
