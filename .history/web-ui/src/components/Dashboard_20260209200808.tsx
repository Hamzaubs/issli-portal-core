...
export const Dashboard = ({ user }: { user?: any }) => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'CASH' | 'CHECK' | 'CREDIT' | 'TRANSFER'>('CASH');
  ...
  const handleTransaction = async () => {
    ...
    if (paymentMethod === 'CREDIT' && !activeClient) {
        alert("⚠️ Client requis pour une vente à crédit.");
    ...
    try {
      ...
      setReceiptData({ 
          ...
          paymentMethod, paymentRef, isReturn: returnMode 
      });
                ...
                <div className="space-y-6">
                    <div className="text-center">
                        ...
                        <div className="flex items-center justify-center gap-4 mt-6">
                            <button onClick={() => setTransactionQty(Math.max(1, transactionQty - 1))} className="w-12 h-12 bg-slate-100 rounded-xl font-black">-</button>
                            <span className="text-4xl font-black">{transactionQty}</span>
                            <button onClick={() => setTransactionQty(transactionQty + 1)} className="w-12 h-12 bg-slate-100 rounded-xl font-black">+</button>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {['CASH', 'CHECK', 'TRANSFER', 'CREDIT'].map((m) => (
                            <button key={m} onClick={() => setPaymentMethod(m as any)} className={`p-3 rounded-xl border-2 text-[10px] font-bold ${paymentMethod === m ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-100'}`}>
                                {m}
                            </button>
                        ))}
                    </div>
                    ...
                    {(paymentMethod === 'CHECK' || paymentMethod === 'TRANSFER') && (
                        <input type="text" placeholder="Référence..." className="w-full p-3 bg-slate-50 border-2 border-slate-200 rounded-xl outline-none font-bold text-sm" value={paymentRef} onChange={e => setPaymentRef(e.target.value)} />
                    )}
                </div>
            ) : (
                <div className="h-full flex flex-col items-center justify-center text-slate-300"><ShoppingCart size={64} className="mb-4"/><p className="font-bold uppercase tracking-widest text-xs">Panier vide</p></div>
            )}
          </div>

          <div className="p-6 border-t">
              <button onClick={handleTransaction} disabled={!selectedProduct || submitting} className="w-full py-4 bg-emerald-600 text-white font-black rounded-2xl shadow-lg disabled:opacity-50">
                  {submitting ? '...' : 'VALIDER TRANSACTION'}
              </button>
          </div>
      </div>
