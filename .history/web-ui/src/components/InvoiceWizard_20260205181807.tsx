...
export const InvoiceWizard: React.FC<Props> = ({ mode, onCancel, onSuccess }) => {
    ...
    const addToCart = (product: any) => {
        ...
        if (existing) {
            ...
            // Ensure product has a VAT rate, default to 20% if missing
    ...
    const updateQuantity = (id: string, delta: number) => {
        setCart(cart.map(item => { if (item.id === id) return { ...item, quantity: Math.max(1, item.quantity + delta) }; return item; }));
    };

    const removeFromCart = (id: string) => setCart(cart.filter(item => item.id !== id));

    // 🛡️ SAFE CALCULATIONS (Matching Backend Logic)
        ...
        try {
            ...
            await client.post('/legal/documents', {
                ...
                items: cart.map(item => ({
                    ...
                    vatRate: safeFloat(item.vatRate) // ✅ Pass strict VAT rate
                })),
                note: note,
