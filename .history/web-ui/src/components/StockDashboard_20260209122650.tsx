...
export const StockDashboard = ({ user }: { user: any }) => {
  ...
  // Helper for Theme Colors (Static Strings for Stability)
  const getTheme = () => {
      if (quoteMode) return { bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-amber-500' };
      if (returnMode) return { bg: 'bg-red-50', text: 'text-red-600', border: 'border-red-600' };
      if (inventoryMode) return { bg: 'bg-purple-50', text: 'text-purple-600', border: 'border-purple-600' };
      return { bg: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-600' };
  };
    ...
    <div className="h-screen flex bg-slate-100 overflow-hidden font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col border-r border-slate-200 bg-[#F8FAFC]">
        <div className="h-20 px-6 bg-white border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
           <div className="flex items-center gap-4">
              <div className={`p-3 rounded-xl transition-colors ${theme.bg} ${theme.text}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {filteredProducts.map(p => (
                        <div key={p.id} onClick={() => { setSelectedProduct(p); setTransactionQty(1); }}
                            ...
                            ${selectedProduct?.id === p.id ? `${theme.border} ring-4 ${quoteMode ? 'ring-amber-500/20' : returnMode ? 'ring-red-500/20' : 'ring-emerald-500/20'}` : 'border-transparent hover:border-slate-300'}`}>
                            ...
                            <div className="mt-auto pt-3 border-t border-slate-50 flex justify-between items-end">
                                ...
                                <div className={`text-xl font-black ${theme.text}`}>{p.sellingPrice} <span className="text-[10px] text-slate-400">DH</span></div>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      {!inventoryMode && !historyMode && (
          <div className="w-[400px] xl:w-[450px] bg-white border-l border-slate-200 shadow-2xl flex flex-col z-20">
              <div className={`p-6 border-b border-slate-100 flex justify-between items-center ${theme.bg}`}>
                  <h2 className={`text-lg font-black uppercase tracking-wide flex items-center gap-2 ${theme.text}`}>
              ...
              <div className="flex-1 p-6 flex flex-col">
                <div className="mb-4">
                  <button onClick={() => setShowClientSelector(true)} className={`w-full p-4 rounded-xl border-2 border-dashed flex items-center justify-between transition-all ${activeClient ? `${theme.border} ${theme.bg} ${theme.text}` : 'border-slate-200 text-slate-400 hover:border-slate-300'}`}>
                   ...
                   <div className="flex-1 flex flex-col animate-in slide-in-from-right-4 duration-300">
                        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 mb-6 flex-1 flex flex-col justify-center text-center relative overflow-hidden">
                            ...
                            <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-inner">
                                <div className="flex justify-between items-center pt-2 mt-2"><span className="text-xs font-bold uppercase text-slate-400">Total à {returnMode ? 'Rendre' : 'Payer'}</span><span className={`text-3xl font-black ${theme.text}`}>{formatMAD(selectedProduct.sellingPrice * transactionQty)}</span></div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 h-20">
                            ...
                            {/* ✅ FIXED: EXPLICIT CONDITIONAL RENDERING TO FORCE TAILWIND JIT CLASSES */}
                            {quoteMode ? (
                                <button 
                                    onClick={handleTransaction} 
                                    disabled={submitting} 
                                    className="h-full rounded-2xl font-black text-xl shadow-xl flex items-center justify-center gap-3 transition-colors text-white bg-amber-500 hover:bg-amber-600"
                                >
                                    {submitting ? '...' : <>AJOUTER <ChevronRight size={24}/></>}
                                </button>
                            ) : returnMode ? (
                                <button 
                                    onClick={handleTransaction} 
                                    disabled={submitting} 
                                    className="h-full rounded-2xl font-black text-xl shadow-xl flex items-center justify-center gap-3 transition-colors text-white bg-red-600 hover:bg-red-700"
                                >
                                    {submitting ? '...' : <>VALIDER <ChevronRight size={24}/></>}
                                </button>
                            ) : (
                                <button 
                                    onClick={handleTransaction} 
                                    disabled={submitting} 
                                    className="h-full rounded-2xl font-black text-xl shadow-xl flex items-center justify-center gap-3 transition-colors text-white bg-emerald-600 hover:bg-emerald-700"
                                >
                                    {submitting ? '...' : <>ENCAISSER <ChevronRight size={24}/></>}
                                </button>
                            )}
                        </div>
                    </div>
