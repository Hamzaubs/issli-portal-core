...
export const StockDashboard = ({ user }: { user: any }) => {
  ...
  // ✅ FIX: Explicit Color Mapping to prevent Tailwind Purge
        ...
        <div className="h-20 px-6 bg-white border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
           <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <p className={`text-xs font-bold uppercase tracking-wider mt-1 text-${theme.title}-500`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {filteredProducts.map(p => (
                        <div key={p.id} onClick={() => { setSelectedProduct(p); setTransactionQty(1); }}
                            ...
                            ${selectedProduct?.id === p.id ? `${theme.border} ring-4 ${theme.ring}` : 'border-transparent hover:border-slate-300'}`}>
                ...
                {selectedProduct ? (
                   <div className="flex-1 flex flex-col animate-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="grid grid-cols-2 gap-4 h-20">
                            ...
                            {/* ✅ BUTTON IS NOW SAFE */}
                            <button onClick={handleTransaction} disabled={submitting} className={`h-full rounded-2xl font-black text-xl shadow-xl flex items-center justify-center gap-3 transition-colors ${theme.btn}`}>
                                {submitting ? '...' : <>{quoteMode ? 'AJOUTER' : returnMode ? 'VALIDER' : 'ENCAISSER'} <ChevronRight size={24} /></>}
                            </button>
                        </div>
                    </div>
                ) : quoteMode ? (
                      // ✅ QUOTE MODE CART VIEW
