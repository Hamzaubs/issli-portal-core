...
// REMOVED: QuotePrinter import (Legacy)
...
export const MovementHistory = () => {
    ...
    const handlePrint = (m: Movement) => {
        // Prepare data for InternalDeliveryNote (Universal Printer for Silo B)
        setPrintTicketData({
                         ...
                         movements.map(m => (
                            <tr key={m.id} className="hover:bg-slate-50/80 transition-colors group">
                                ...
                                <td className="p-4 text-right flex justify-end gap-2">
                                    <button onClick={() => handlePrint(m)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Imprimer Ticket"><Printer size={16}/></button>
