...
export const LegalAnalytics: React.FC<AnalyticsProps> = ({ onBack }) => {
    ...
    // Scale Logic (Safe Math)
    const maxVal = Math.max(...(data.charts.monthly || []).map((m: any) => Math.max(Number(m.revenue || 0), Number(m.refunds || 0))), 1000);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-2xl border-l-4 border-l-emerald-500 shadow-sm print-shadow-none">
                    ...
                    <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden mt-2">
                        <div className="bg-emerald-500 h-full" style={{ width: `${Math.min(Number(kpi.marginRate || 0), 100)}%` }}></div>
                    </div>
                    <p className="text-[10px] text-right mt-1 text-emerald-600 font-bold">{Number(kpi.marginRate || 0).toFixed(1)}% Rentabilité</p>
                </div>
            </div>
                            ...
                            {data.charts.topProducts.map((p: any, i: number) => (
                                <div key={i} className="group">
                                    <div className="flex justify-between text-[11px] font-bold mb-1 text-slate-600">
                                        ...
                                        <span className="font-mono text-slate-900">{Number(p.total).toFixed(1)} u</span>
                                    </div>
