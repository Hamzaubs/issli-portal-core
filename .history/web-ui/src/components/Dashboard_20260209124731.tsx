...
import { 
  ...
  ChevronRight, FileText, PieChart
} from 'lucide-react';
...
import { ExecutiveDashboard } from './ExecutiveDashboard'; // Kept for Admin view
...
export const Dashboard = ({ user }: { user?: any }) => {
  ...
  useEffect(() => {
    if (viewMode === 'OPERATIONAL') {
        // ✅ Use the new Safe Internal Route
  ...
  const handleTransaction = async () => {
    ...
    setSubmitting(true);
    try {
      // ✅ Use the new Safe Transaction Route
              ...
              <div className="flex justify-between items-center mb-8">
                  ...
                  <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => setViewMode('OPERATIONAL')} className="flex items-center gap-2 px-4 py-2 bg-white text-slate-700 font-bold rounded-xl border border-slate-200 hover:bg-slate-50 shadow-sm transition-all">
                          <Anchor size={18}/> Vue Opérationnelle
                      </button>
                  </div>
              </div>
      ...
      <div className="flex-1 flex flex-col border-r border-slate-200 bg-[#F8FAFC]">
        <div className="h-20 px-6 bg-white border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
           ...
           <div className="flex items-center gap-3">
               ...
               {/* ✅ QUOTE WIZARD BUTTON */}
               <button onClick={() => setShowQuoteWizard(true)} className="px-4 py-3 rounded-xl font-bold text-sm bg-amber-50 text-amber-600 border border-amber-200 hover:bg-amber-100 transition-all flex items-center gap-2 shadow-sm">
