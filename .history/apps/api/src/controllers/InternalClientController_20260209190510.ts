...
export const InternalClientController = {
  ...
  getClientDetails: async (req: Request, res: Response) => {
    try {
      ...
      const stats = await prismaInternal.stockMovement.aggregate({
        where: { clientId: id, type: { in: ['SALE_CASH', 'RETURN'] } }, // Raw strings work fine here
        _sum: { amount: true },
        _max: { createdAt: true }
      });
  ...
  getClientHistory: async (req: Request, res: Response) => {
    try {
      ...
      // ✅ FIX 1: Cast type to 'any' to bypass strict Enum check (safe because filtering is optional)
      ...
      const history = movements.map(m => ({
        ...
        // ✅ FIX 3: Removed paymentRef (Add column to schema later to enable this)
