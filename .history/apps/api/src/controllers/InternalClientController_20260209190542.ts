...
export const InternalClientController = {
  ...
  getClientHistory: async (req: Request, res: Response) => {
    try {
      ...
      // ✅ FIX 1: Cast type to 'any' to bypass strict Enum check
      ...
      const history = movements.map(m => ({
        ...
        // ✅ FIX 2: Safe number conversion for nullable amount
        amount: m.amount?.toNumber() ?? 0,
        paymentMethod: m.paymentMethod,
        // ✅ FIX 3: Manually access paymentRef if it exists on the record
        paymentRef: (m as any).paymentRef || null 
      }));
  ...
  // 💰 NEW: REGISTER PAYMENT (Decreases Debt)
  registerPayment: async (req: Request, res: Response) => {
    try {
      ...
      await prismaInternal.stockMovement.create({
        data: {
          type: 'PAYMENT' as any, // Cast as any if Enum doesn't strictly have PAYMENT yet
          clientId: id,
          amount: new Prisma.Decimal(amount).neg(), // Payments are negative (Credit)
          quantity: 0,
          paymentMethod: method || 'CASH',
          paymentRef: ref || null,
          productId: '00000000-0000-0000-0000-000000000000', // Dummy product ID or null if allowed
          unitPrice: 0
        }
      });
