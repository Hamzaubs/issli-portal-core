...
export const LegalReportController = {
  ...
  getExport: async (req: Request, res: Response) => {
    try {
        ...
        // 🛡️ Helper to clean strings for CSV (Remove semicolons)
        ...
        if (type === 'journal') {
            ...
            invoices.forEach(inv => {
                const ht = round(toDec(inv.totalHT));
                const ttc = round(toDec(inv.totalTTC));
                const tva = round(toDec(inv.totalTTC).sub(toDec(inv.totalHT)));
                const status = inv.status === 'PAYEE' ? 'Payee' : 'En Attente';
                csvContent += `${fmtDate(inv.issuedAt)};${clean(inv.reference)};${clean(inv.clientNameSnapshot)};${clean(inv.clientIceSnapshot)};${ht};${tva};${ttc};${status}\n`;
            });
        ...
        // 2. RELEVE ENCAISSEMENTS (Cash Flow)
            ...
            csvContent = "Date;Reference Paiement;Client;Facture Liee;Mode;Montant;Note\n";
            payments.forEach(pay => {
                csvContent += `${fmtDate(pay.paidAt)};${clean(pay.reference || "-")};${clean(pay.invoice?.clientNameSnapshot)};${clean(pay.invoice?.reference)};${clean(pay.method)};${round(toDec(pay.amount))};${clean(pay.note)}\n`;
            });
        ...
        // 3. INVENTAIRE (Stock Value)
            ...
            products.forEach(p => {
                ...
                csvContent += `${clean(p.serialNumber)};${clean(p.name)};${round(qty)};${clean(p.measureUnit)};${round(cost)};${round(toDec(p.priceHT))};${round(val)}\n`;
            });
        ...
        // 4. BILAN (Simplified)
            ...
            // Re-use logic for speed (Simplified)
            csvContent = "METRIQUE;VALEUR (MAD)\n";
            csvContent += `Generated;${new Date().toISOString()}\n`;
            csvContent += `Note;Ce document est une extraction brute pour analyse.\n`;
