...
export const InvoicesController = {
    ...
    try {
      ...
      const result = await prismaLegal.$transaction(async (tx) => {
          ...
          for (const item of items) {
              ...
              const rate = new Prisma.Decimal(item.vatRate || 0.20); // Dynamic VAT (10% or 20%)
          ...
          if (type === 'FACTURE') {
             ...
             // Shared sequence for all Credit Notes (Manual or Cancelled) using year 9999
             const seq = await tx.invoiceSequence.upsert({ 
                where: { year: 9999 }, 
                update: { lastCount: { increment: 1 } },
          ...
          if (type === 'AVOIR') status = 'PAYEE'; // Credit notes are "settled" immediately usually
          if (type === 'FACTURE' && initialPayment >= Number(totalTTC)) status = 'PAYEE';
    ...
    try {
        ...
        const result = await prismaLegal.$transaction(async (tx) => {
            ...
            for (const item of returnedItems) {
                ...
                // 1. Re-create/Create Product in Stock
            ...
            for (const item of newItems) {
                ...
                const lineHT = price.mul(item.quantity);
                const lineTTC = lineHT.mul(rate.add(1));
        ...
        }, { timeout: 15000 }); // Extended timeout for large ops
  ...
  // Legacy alias for routes calling createCreditNote
