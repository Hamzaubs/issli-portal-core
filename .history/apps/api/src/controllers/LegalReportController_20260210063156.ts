    ...
    try {
        ...
        if (type === 'journal') {
            ...
            invoices.forEach(inv => {
                const ht = formatExcelNum(toDec(inv.totalHT));
                const ttc = formatExcelNum(toDec(inv.totalTTC));
                const tva = formatExcelNum(toDec(inv.totalTTC).sub(toDec(inv.totalHT)));
        ...
        // 2. RELEVE ENCAISSEMENTS (Cash Flow)
            ...
            payments.forEach(pay => {
                csvContent += `${fmtDate(pay.paidAt)};${clean(pay.reference || "-")};${clean(pay.invoice?.clientNameSnapshot)};${clean(pay.invoice?.reference)};${clean(pay.method)};${formatExcelNum(toDec(pay.amount))};${clean(pay.note)}\n`;
            });
