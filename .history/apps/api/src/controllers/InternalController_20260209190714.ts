...
export const InternalController = {
  ...
  // 💰 CREATE TRANSACTION (With Debt Logic)
  createTransaction: async (req: Request, res: Response) => {
    try {
      ...
      if (type === 'RETURN' || type === 'RESTOCK' || type === 'ADJUSTMENT') {
        ...
        // 🟢 DEBT LOGIC FOR RETURN
        if (type === 'RETURN' && clientId) {
           // If return, we decrease the total spent (refund volume)
           // If originally bought on credit (or generally), we decrease debt.
      ...
      operations.push(prismaInternal.stockMovement.create({
          data: {
            ...
            paymentMethod: paymentMethod || 'CASH', paymentRef: paymentRef || null, amount: financialImpact, 
            snapshotPurchaseCost: product.purchaseCost, snapshotSellingPrice: product.sellingPrice, snapshotProductName: product.name
          }
        })
      );
