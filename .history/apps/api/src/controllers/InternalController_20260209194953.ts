...
export const InternalController = {
  ...
  // 📦 PRODUCT CRUD (Kept same for brevity)
  ...
  // ✅ CREATE TRANSACTION (With Debt + PaymentRef)
  createTransaction: async (req: Request, res: Response) => {
    try {
      const { productId, userId, quantity, type, clientId, paymentMethod, paymentRef } = req.body; 
      ...
      if (type === 'RETURN' || type === 'RESTOCK' || type === 'ADJUSTMENT') {
        ...
        if (type === 'RETURN' && clientId) {
           operations.push(prismaInternal.clientB.update({ where: { id: clientId }, data: { balance: { decrement: totalAmount }, totalSpent: { decrement: totalAmount } } }));
        }
      }
      ...
      operations.push(prismaInternal.stockMovement.create({
          data: {
            ...
            paymentRef: paymentRef || null, // ✅ SAVING TO NATIVE COLUMN
  ...
  createBatchTransaction: async (req: Request, res: Response) => {
    ...
    } catch (e: any) { res.status(500).json({ error: e.message || "Erreur enregistrement devis" }); }
  },
  ...
  voidTransaction: async (req: Request, res: Response) => {
    try {
      ...
      if (original.type === 'SALE_CASH') { reverseType = MovementType.RETURN; stockMultiplier = 1; } 
      else if (original.type === 'RETURN') { reverseType = MovementType.SALE_CASH; stockMultiplier = -1; } 
      else { reverseType = MovementType.ADJUSTMENT; stockMultiplier = -1; }
    ...
    } catch (e) { res.status(500).json({ error: "Erreur lors de l'annulation" }); }
  }
};