...
export const InternalController = {
  ...
  createBatchTransaction: async (req: Request, res: Response) => {
    try {
      ...
      const operations: any[] = [];
      const timestamp = new Date(); // ✅ Shared timestamp groups the items as one "Document"
  ...
  // =================================================================
  // 🔄 CONVERSION ENGINE (Smart Quote -> Sale)
  // =================================================================
  convertQuoteToSale: async (req: Request, res: Response) => {
    try {
      const { quoteId, paymentMethod } = req.body; // quoteId can be ANY movement ID from the batch
      const userId = (req as any).user?.id;

      // 1. Find the trigger item
      const triggerItem = await prismaInternal.stockMovement.findUnique({ where: { id: quoteId } });
      if (!triggerItem) return res.status(404).json({ error: "Ligne de devis introuvable" });

      // 2. Find ALL siblings (Same timestamp + Client + Quote Type) -> Reconstitutes the "Document"
      const quoteLines = await prismaInternal.stockMovement.findMany({
        where: { 
            createdAt: triggerItem.createdAt, 
            clientId: triggerItem.clientId,
            type: MovementType.QUOTE 
        },
        include: { product: true }
      });

      if (quoteLines.length === 0) return res.status(404).json({ error: "Aucun devis valide trouvé" });
      ...
      for (const line of quoteLines) {
         ...
         operations.push(prismaInternal.stockMovement.create({
            data: {
               productId: line.productId, userId, clientId: line.clientId, quantity: line.quantity,
               type: MovementType.SALE_CASH, paymentMethod: paymentMethod || 'CASH',
               amount: line.amount, // Preserves quoted price
      ...
      await prismaInternal.$transaction(operations);
      res.json({ success: true, message: "Devis intégral converti en vente !" });
  ...
  voidTransaction: async (req: Request, res: Response) => {
    try {
      ...
      let reverseType = original.type === 'SALE_CASH' ? MovementType.RETURN : MovementType.SALE_CASH;
      if (original.type === 'RESTOCK' || original.type === 'ADJUSTMENT') reverseType = MovementType.ADJUSTMENT;

      const stockMultiplier = original.type === 'SALE_CASH' ? 1 : -1;
      ...
      operations.push(prismaInternal.stockMovement.create({
          data: {
              ...
              quantity: original.quantity, type: reverseType as MovementType, paymentMethod: original.paymentMethod,
