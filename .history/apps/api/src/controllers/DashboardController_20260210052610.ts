...
// Converts Prisma Decimals, Nulls, and Strings to safe JS Numbers
const toNumber = (val: any) => {
    if (!val) return 0;
    if (typeof val === 'object' && 'toNumber' in val) return val.toNumber(); // Handle Decimal
    return Number(val);
};
...
export const DashboardController = {
  ...
  getGlobalStats: async (req: Request, res: Response) => {
      try {
          ...
          // 🛡️ Bypass Enum check with 'as any' if needed, but imported MovementType is better
          ...
          invoicesA.forEach(inv => {
              ...
              inv.items.forEach(item => {
                  // ✅ FIX: Wrap both Cost and Quantity in toNumber()
                  invCost += (toNumber(item.unitPurchaseCostSnapshot) * toNumber(item.quantity));
              });
          ...
          movementsB.forEach(mov => {
              ...
              // ✅ FIX: Wrap Cost and Quantity
              const cost = toNumber(mov.snapshotPurchaseCost) * toNumber(mov.quantity);
          ...
          // ✅ FIX: Use toNumber() inside reduce loop
          const valueStockA = productsA.reduce((acc, p) => acc + (toNumber(p.purchaseCost) * toNumber(p.quantity)), 0);
          const valueStockB = productsB.reduce((acc, p) => acc + (toNumber(p.purchaseCost) * toNumber(p.quantity)), 0);
          ...
          res.json({
              metrics: {
                  ...
                  treasury: {
                      realCash: realCash,
                      checks: Math.abs(toNumber(checksOnHand._sum.amount)),
  ...
  getProducts: async (req: Request, res: Response) => {
    try {
      ...
      const safe = products.map(p => ({
          ...
          quantity: toNumber(p.quantity) // ✅ Fix
      }));
      res.json(safe);
  ...
  getGlobalClientDetails: async (req: Request, res: Response) => {
     try {
         ...
         res.json({ 
             ...
             history: movements.map(m => ({ ...m, amount: toNumber(m.amount), quantity: toNumber(m.quantity) })) // ✅ Fix
         });
  ...
  getHistory: async (req: Request, res: Response) => {
    try {
        ...
        const formatted = movements.map(m => ({
            ...
            quantity: toNumber(m.quantity), // ✅ Fix
         ...
         await prismaInternal.$transaction(async (tx) => {
             ...
             await tx.stockMovement.create({
                 data: {
                     ...
                     snapshotSellingPrice: toNumber(product.sellingPrice),
                     snapshotPurchaseCost: toNumber(product.purchaseCost)
                 }
             });
         });
         res.json({ success: true });
