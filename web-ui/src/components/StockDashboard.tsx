// web-ui/src/components/StockDashboard.tsx
...
export const StockDashboard = ({ user }: { user: any }) => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'CASH' | 'CHECK' | 'CREDIT' | 'QUOTE'>('CASH');
  ...
  const handleTransaction = async () => {
    ...
    if ((paymentMethod === 'CREDIT' || paymentMethod === 'QUOTE') && !activeClient) {
        alert("⚠️ Action impossible : Vous devez sélectionner un CLIENT pour un Crédit ou un Devis.");
  ...
  return (
    <div className="h-screen flex bg-slate-100 overflow-hidden font-sans text-slate-900 relative">
      ...
      <div className="flex-1 flex flex-col border-r border-slate-200 bg-[#F8FAFC]">
        <div className="h-20 px-6 bg-white border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
           ...
           <div className="flex items-center gap-3">
               {!historyMode && !inventoryMode && !analyticsMode && (
               ...
               {user.role === 'SUPER_ADMIN' && (
                   <>
                       <button onClick={() => { setAnalyticsMode(true); setHistoryMode(false); setInventoryMode(false); }} className={`p-3 rounded-xl border-2 transition-all ${analyticsMode ? 'bg-emerald-700 text-white border-emerald-700' : 'bg-white text-slate-500 border-slate-200 hover:border-emerald-400'}`} title="Analytique">
                           <TrendingUp size={20} />
                       </button>
                       <button onClick={() => { setHistoryMode(!historyMode); setInventoryMode(false); setAnalyticsMode(false); }} className={`p-3 rounded-xl border-2 transition-all ${historyMode ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-400'}`}>
                           <History size={20} />
                       </button>
                   </>
               )}
               
               {/* HEADER DEVIS BUTTON (Optional, kept for quick access) */}
               ...
               <button onClick={() => { setReturnMode(!returnMode); setSelectedProduct(null); setAnalyticsMode(false); setPaymentMethod('CASH'); }} className={`px-4 py-3 rounded-xl font-bold text-sm transition-all shadow-sm border ${returnMode ? 'bg-red-600 text-white border-red-600' : 'bg-white text-slate-600 border-slate-200 hover:border-red-300'}`}>RETOUR</button>

               {user.role === 'SUPER_ADMIN' && !historyMode && !inventoryMode && !analyticsMode && <button onClick={() => { setEditingProduct(null); setShowFormModal(true); }} className="p-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-500 transition-colors shadow-lg shadow-emerald-200"><Plus size={20} /></button>}
        ...
        <div className="flex-1 overflow-auto p-6 relative">
            {analyticsMode ? <InternalAnalytics onBack={() => setAnalyticsMode(false)} /> :
             historyMode ? <MovementHistory /> : 
             inventoryMode ? <InventorySheet products={products} mode="internal" onClose={() => setShowInventorySheet(false)} /> : (
      ...
      {/* RIGHT SIDE: CART */}
      {!inventoryMode && !historyMode && !analyticsMode && (
                   ...
                   <div className="flex-1 flex flex-col animate-in slide-in-from-right-4 duration-300">
                        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 mb-6 flex-1 flex flex-col justify-center text-center relative overflow-hidden">
                            ...
                            <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-inner">
                                <div className="flex justify-between items-center pt-2 mt-2"><span className="text-xs font-bold uppercase text-slate-400">Total à {returnMode ? 'Rendre' : paymentMethod === 'QUOTE' ? 'Estimer' : 'Payer'}</span><span className={`text-3xl font-black ${returnMode ? 'text-red-600' : 'text-emerald-600'}`}>{formatMAD(selectedProduct.sellingPrice * transactionQty)}</span></div>
                            </div>
                        </div>

                        {/* ✅ PAYMENT / QUOTE SELECTOR */}
                        {!returnMode && (
                            <div className="bg-white p-3 rounded-xl border border-slate-200 mb-4 shadow-sm">
                                <div className="grid grid-cols-3 gap-2">
                                    <button onClick={() => setPaymentMethod('CASH')} className={`p-2 rounded-lg text-[10px] font-bold border flex items-center justify-center gap-1 transition-all ${paymentMethod === 'CASH' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'border-slate-200 text-slate-500'}`}>💵 Espèces</button>
                                    <button onClick={() => setPaymentMethod('CREDIT')} className={`p-2 rounded-lg text-[10px] font-bold border flex items-center justify-center gap-1 transition-all ${paymentMethod === 'CREDIT' ? 'bg-red-50 border-red-500 text-red-700' : 'border-slate-200 text-slate-500'}`}>⏳ Crédit</button>
                                    <button onClick={() => setPaymentMethod('QUOTE')} className={`p-2 rounded-lg text-[10px] font-bold border flex items-center justify-center gap-1 transition-all ${paymentMethod === 'QUOTE' ? 'bg-amber-50 border-amber-500 text-amber-700' : 'border-slate-200 text-slate-500'}`}>📝 Devis (Rapide)</button>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-4 h-20">
                            ...
                            <button 
                                ...
                                disabled={submitting} 
                                className={`h-full rounded-2xl font-black text-lg shadow-xl flex items-center justify-center gap-3 transition-colors text-white ${returnMode ? 'bg-red-600 hover:bg-red-700' : paymentMethod === 'QUOTE' ? 'bg-amber-500 hover:bg-amber-600' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                            >
                                {submitting ? '...' : <>{returnMode ? 'VALIDER' : paymentMethod === 'QUOTE' ? 'GÉNÉRER DEVIS' : 'ENCAISSER'} <ChevronRight size={20} /></>}
                            </button>
                        </div>
                    </div>
                ) : (
                    // ✅ NEW: MASSIVE DEVIS BUTTON IN EMPTY CART
                    <div className="flex-1 flex flex-col items-center justify-center relative">
                        <div className="text-slate-300 opacity-50 flex flex-col items-center mb-8">
                            <ShoppingCart size={80} strokeWidth={1} className="mb-4" />
                            <p className="text-lg font-bold uppercase tracking-widest text-center">En attente de produit</p>
                        </div>
                        
                        <div className="w-full px-6 flex flex-col gap-4 absolute bottom-6">
                            <div className="text-center relative">
                                <span className="bg-white px-3 text-xs font-bold text-slate-300 uppercase tracking-widest relative z-10">Options</span>
                                <div className="absolute top-1/2 left-0 right-0 h-px bg-slate-100"></div>
                            </div>
                            <button onClick={() => setShowQuoteWizard(true)} className="w-full py-5 bg-amber-50 border-2 border-dashed border-amber-200 text-amber-600 hover:bg-amber-100 hover:border-amber-400 hover:text-amber-700 rounded-2xl font-black shadow-sm flex items-center justify-center gap-3 transition-all hover:scale-[1.02]">
                                <FileText size={24}/> CRÉER UN DEVIS (MULTI-ARTICLES)
                            </button>
                        </div>
                    </div>
                )}
