...
export const SupplierStatementPrint = ({ supplier, statement, onClose }: SupplierStatementPrintProps) => {
  const componentRef = useRef(null);
  const handlePrint = useReactToPrint({ contentRef: componentRef, documentTitle: `Relevé_${supplier.name}` });
            ...
            <table className="w-full mb-8 relative z-10">
              <thead className="bg-slate-50 text-slate-700 text-[10px] font-bold uppercase border-y border-slate-200">
                <tr>
                    ...
                    <th className="py-3 px-3 text-left">Opération & Référence</th>
                    <th className="py-3 px-3 text-right">Débit (Règlements)</th>
                    <th className="py-3 px-3 text-right">Crédit (Achats)</th>
                    <th className="py-3 px-3 text-right">Solde</th>
                </tr>
              </thead>
                ...
                {statement.length === 0 ? (
                    ...
                    <tr key={item.id || index} className="border-b border-slate-50">
                        ...
                        <td className="py-3 px-3">
                            ...
                            <div className="text-[9px] text-slate-500 uppercase">{item.type.replace('_', ' ')}</div>
                        </td>
