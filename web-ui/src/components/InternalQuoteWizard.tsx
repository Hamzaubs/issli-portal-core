// web-ui/src/components/InternalQuoteWizard.tsx
import React, { useState, useEffect, useCallback } from 'react';
import { 
    X, Search, User, ShoppingCart, 
    FileText, ArrowRight, ChevronRight, Calculator, UserPlus, Trash2, CheckCircle2
} from 'lucide-react';
...
export const InternalQuoteWizard: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const handleCreateClient = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStep(2);
        } catch (e: any) { alert("Erreur: " + e.response?.data?.error); } 
        finally { setLoading(false); }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            await client.post('/internal/transactions/batch', {
                ...
                paymentMethod: processMode === 'SALE' ? paymentMethod : undefined,
                items: cart.map(item => ({
                    productId: item.id,
            ...
            alert(`✅ ${processMode === 'QUOTE' ? 'Devis' : 'Vente BL'} enregistré avec succès !`);
            onSuccess(); 
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-slate-800 leading-tight flex items-center gap-2">
                                CAISSE & PANIER <span className="text-[10px] px-2 py-0.5 rounded-full border bg-amber-50 text-amber-700 border-amber-200 uppercase tracking-wide">Stock B</span>
                            </h2>
                            <div className="flex items-center gap-2 text-xs font-medium text-slate-400 mt-1">
                                <span className={step === 1 ? `text-amber-600 font-bold` : ''}>1. Client</span>
                                <ArrowRight size={10}/>
                                <span className={step === 2 ? `text-amber-600 font-bold` : ''}>2. Articles & Paiement</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="w-full md:w-[35%] bg-slate-50 flex flex-col border-l border-white shadow-2xl relative z-10">
                            ...
                            <div className="p-5 bg-white border-t border-slate-200 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
                                <div className="flex gap-2 mb-4 p-1 bg-slate-100 rounded-lg">
                                    ...
                                    <button onClick={() => setProcessMode('SALE')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-1 ${processMode === 'SALE' ? 'bg-emerald-600 shadow text-white' : 'text-slate-500 hover:text-slate-700'}`}>
                                        <CheckCircle2 size={12}/> VENTE BL
                                    </button>
                                </div>
                                ...
                                <button onClick={handleSubmit} disabled={loading || cart.length === 0} 
                                    className={`w-full py-4 text-white font-black rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all ${processMode === 'SALE' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-slate-900 hover:bg-slate-800'}`}>
                                    {loading ? 'Enregistrement...' : <>ENREGISTRER <Calculator size={18}/></>}
                                </button>
                            </div>
                        </div>
                    </div>
