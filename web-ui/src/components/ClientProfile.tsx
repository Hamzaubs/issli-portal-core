...
import { 
    ...
    Loader2, Banknote, User, Wallet, RotateCcw, 
    ArrowUpRight, FileText, ChevronDown, ShoppingCart, History
} from 'lucide-react';
import client from '../api/client';
import { ClientStatement } from './ClientStatement';
import { InternalDeliveryNote } from './InternalDeliveryNote';
// ✅ NEW: Import the Payment Receipt component
...
// 🛡️ INTERNAL TARGETED PAYMENT MODAL
const InternalPaymentModal: React.FC<{ 
    ...
    const defaultAmount = targetTicket ? (targetTicket.amount - targetTicket.paid) : maxAmount;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (payAmount > maxAmount) return alert(`Impossible de payer plus que la dette actuelle (${maxAmount} MAD).`);
        ...
        try {
            await client.post(`/internal/clients/${clientId}/payment`, { 
                ...
                movementId: targetTicket ? targetTicket.id : undefined 
            });
...
export const ClientProfile: React.FC<Props> = ({ clientId, onClose }) => {
    ...
    const [paymentPrintData, setPaymentPrintData] = useState<any>(null); // ✅ NEW: State for Receipt
    ...
    // ✅ FIX: Intelligently routes the print job based on Movement Type
    const handlePrint = (m: any) => {
        if (m.type === 'PAYMENT') {
            setPaymentPrintData({
                ...
                note: m.productName, // Translates to "Règlement Dette..."
                context: 'MAGASIN INTERNE (SILO B)' 
            });
            return;
        }

        // Default Delivery Note routing for Sales/Returns
        setPrintData({
            id: m.id.substring(0, 8).toUpperCase(), 
            productName: m.productName.replace('Vente - ', ''), 
            sku: m.sku, 
            quantity: m.quantity, 
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95">
            <div className="bg-slate-100 w-full max-w-5xl h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-700 relative">
                    ...
                    <div className="bg-white p-6 flex flex-col justify-between group relative overflow-hidden col-span-2">
                        <div className="flex justify-between items-start relative z-10">
                            <div>
                                ...
                                <p className={`text-4xl font-black mt-2 ${profile?.balance > 0 ? 'text-red-600' : 'text-slate-800'}`}>{formatMAD(profile?.balance || 0)}</p>
                            </div>
                            ...
                            <div className="flex flex-col gap-2">
                                <button 
                                    ...
                                    className={`px-4 py-2 rounded-xl transition-colors font-bold flex items-center justify-center gap-2 ${profile?.balance > 0 ? 'bg-red-600 text-white hover:bg-red-700 shadow-lg shadow-red-200' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}>
                                    <Banknote size={20}/> {profile?.balance > 0 ? 'ENCAISSER' : 'À JOUR'}
                                </button>
                ...
                {/* HISTORY LIST WITH EXACT TICKET TARGETING */}
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-100 custom-scrollbar">
                      {history.map((m, idx) => {
                          ...
                          return (
                              <div key={`${m.id}-${idx}`} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center transition-all hover:border-emerald-300 group">
                                  <div className="flex items-center gap-4">
                                      <div>
                                          <div className="font-bold text-slate-800 text-sm mb-1 flex items-center gap-2">
                                              ...
                                              {/* DYNAMIC DEBT LABELS */}
                                              {(m.type === 'SALE_CASH' || m.type === 'SALE_CREDIT') && (
                                                  isFullyPaid ? (
                                                      ...
                                                      <span className="text-[9px] bg-sky-100 text-sky-700 px-1.5 rounded border border-sky-200 font-bold uppercase flex items-center gap-1"><PieChart size={8}/> Reste {remaining} DH</span>
                                                  ) : (
                                          ...
                                          <div className="flex gap-2">
                                              {getTypeBadge(m.type)}
                                              {m.paymentMethod && <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-bold uppercase border border-slate-200">{m.paymentMethod}</span>}
                                          </div>
                                          <div className="text-[10px] text-slate-400 font-mono mt-1.5">
                                              {new Date(m.date).toLocaleString('fr-MA')} <span className="mx-1">•</span> Réf: {m.id.substring(0,8)}
                                          </div>
                                      </div>
                                  </div>
                                  <div className="text-right flex items-center gap-4">
                                      <div className={`font-black ${m.type === 'RETURN' || m.type === 'PAYMENT' ? 'text-emerald-600' : 'text-slate-800'}`}>
                                          {m.type === 'RETURN' || m.type === 'PAYMENT' ? '+' : ''}{formatMAD(m.amount)}
                                      </div>
                                      
                                      {/* TARGETED PAYMENT BUTTON */}
                                      {(m.type === 'SALE_CASH' || m.type === 'SALE_CREDIT') && !isFullyPaid ? (
                                          <button onClick={() => setSelectedDebtTicket(m)} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg shadow-sm flex items-center gap-1 transition-all hover:scale-105">
                                      ...
                                      {/* ✅ FIX: Removed 'PAYMENT' from the blocking condition */}
                                      {m.type !== 'QUOTE' && (
                                          <button onClick={() => handlePrint(m)} className="p-1.5 bg-slate-50 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors border border-transparent">
                                              <Printer size={16}/>
                                          </button>
                                      )}
                                  </div>
                ...
                {selectedDebtTicket && (
                    <InternalPaymentModal 
                        clientId={clientId} 
                        clientName={profile?.name || ''}
                        maxAmount={selectedDebtTicket === "GLOBAL" ? profile?.balance : (selectedDebtTicket.amount - selectedDebtTicket.paid)} 
